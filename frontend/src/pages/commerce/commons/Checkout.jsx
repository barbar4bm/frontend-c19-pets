...
export const Checkout = () => {
    ...
    const saveShipping = async (data) => {
        console.log(data);
        try {
            const response = await axios.post('http://localhost:3001/api/Shipping/',data).then((res)=>{
        ...
        <>
            ...
            <article>
                <section className='details' style={{ alignItem: 'center' }}>
                    ...
                    <h3 style={{ color: "red", marginLeft:50 }}>Precio Total : $ {price}</h3>

                    <button className='button' style={{ padding: 20, marginBottom: 5, marginLeft:50 }} onClick={openModal}>Envio</button>
                    ...
                    <button disabled={!activeButton} style={{alignSelf:'center',marginLeft:50}} className='button' onClick={() => pay(carrito)} >Pagar</button>
                </section>
            </article>
        </>
    )
